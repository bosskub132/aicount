...
function PurchaseVatContent() {
  ...
  const { data, isLoading: queryLoading } = useVatRegister({ period, direction: "EXPENSE" });
  const isLoading = !mounted || queryLoading;
  const reportData = data as VatRegisterData | undefined;

  const history = useReportHistory({ reportType: "purchase-vat" });
  ...
  const statCards = useMemo(() => [
    { label: "Total Purchases", value: formatCurrency(reportData?.totalPurchases ?? 0), color: "border-l-[var(--primary)]" },
    { label: "Total Input VAT", value: formatCurrency(reportData?.totalInputVat ?? 0), color: "border-l-[var(--success)]" },
    { label: "# Invoices", value: String(reportData?.invoiceCount ?? 0), color: "border-l-[var(--secondary)]" },
    { label: "# Vendors", value: String(reportData?.vendorCount ?? 0), color: "border-l-[var(--warning)]" },
  ], [reportData]);
  ...
  return (
    <section className="space-y-5 p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-[var(--foreground)]">Purchase VAT Register</h1>
          <p className="text-sm text-[var(--muted-foreground)]">{"\u0e23\u0e32\u0e22\u0e07\u0e32\u0e19\u0e20\u0e32\u0e29\u0e35\u0e0b\u0e37\u0e49\u0e2d"} &mdash; Input VAT register for purchases</p>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title="No purchase VAT data"
          description="There are no purchase VAT transactions for the selected period."
        />
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border)] bg-[var(--muted)]">
                  ...
                  <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">{"\u0e0a\u0e37\u0e48\u0e2d\u0e1c\u0e39\u0e49\u0e02\u0e32\u0e22"}</th>
              ...
              <tbody>
                {paginatedLines.map((line) => (
                  <tr key={line.seq} className="border-b border-[var(--muted)] hover:bg-[var(--muted)]/30">
                    ...
                    <td className="px-3 py-2.5 text-[var(--foreground)]">{line.vendorName}</td>
              ...
              <tfoot>
                <tr className="border-t-2 border-t-[var(--foreground)] bg-[var(--muted)] font-bold">
                  ...
                  <td className="px-3 py-3 text-right tabular-nums">{formatCurrency(reportData?.totalPurchases ?? 0)}</td>
                  <td className="px-3 py-3 text-right tabular-nums">{formatCurrency(reportData?.totalInputVat ?? 0)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
...
export default function PurchaseVatClient() {
  return (
