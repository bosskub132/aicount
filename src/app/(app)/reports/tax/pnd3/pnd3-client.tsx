...
import { useTaxPnd3 } from "@/lib/hooks/use-tax-pnd3";
...
function Pnd3Content() {
  ...
  const { data, isLoading: queryLoading } = useTaxPnd3({ period });
  const isLoading = !mounted || queryLoading;
  const reportData = data as Pnd3Data | undefined;

  const history = useReportHistory({ reportType: "pnd3" });
  ...
  const statCards = useMemo(() => [
    ...
    { label: "Most Common Rate", value: reportData?.mostCommonRate ?? "-", color: "border-l-[var(--warning)]" },
  ], [reportData]);
  ...
  return (
    <section className="space-y-5 p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-[var(--foreground)]">{"\u0e20.\u0e07.\u0e14.3"} Individual WHT</h1>
          <p className="text-sm text-[var(--muted-foreground)]">{"\u0e2b\u0e31\u0e01 \u0e13 \u0e17\u0e35\u0e48\u0e08\u0e48\u0e32\u0e22\u0e1a\u0e38\u0e04\u0e04\u0e25\u0e18\u0e23\u0e23\u0e21\u0e14\u0e32"} &mdash; Withholding tax for individual payees</p>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title="No PND3 data"
          description="There are no individual WHT transactions for the selected period."
        />
      ) : (
        <>
...
export default function Pnd3Client() {
  return (
