...
export async function GET(
  ...
  try {
    const data = await getPp30(id, period);
    return NextResponse.json({ success: true, data });
  } catch (err) {
    console.error("[tax/pp30] query failed:", err);
    return NextResponse.json({ success: false, error: "Failed to generate PP30 report" }, { status: 500 });
  }
}
