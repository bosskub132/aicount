...
export async function GET(
  ...
  try {
    const data = await getPnd53(id, period);
    return NextResponse.json({ success: true, data });
  } catch (err) {
    console.error("[tax/pnd53] query failed:", err);
    return NextResponse.json({ success: false, error: "Failed to generate PND53 report" }, { status: 500 });
  }
}
