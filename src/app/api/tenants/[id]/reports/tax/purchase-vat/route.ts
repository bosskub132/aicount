...
export async function GET(
  ...
  try {
    const data = await getVatRegister(id, period, "EXPENSE");
    return NextResponse.json({ success: true, data });
  } catch (err) {
    console.error("[tax/purchase-vat] query failed:", err);
    return NextResponse.json({ success: false, error: "Failed to generate purchase VAT register" }, { status: 500 });
  }
}
