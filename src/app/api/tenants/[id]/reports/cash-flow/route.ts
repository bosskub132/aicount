...
export async function GET(
  ...
  try {
    const data = await getCashFlow(id, { period, scope, comparison });
    return NextResponse.json({ success: true, data });
  } catch (err) {
    console.error("[reports/cash-flow] Error:", err);
    return NextResponse.json({ success: false, error: "Failed to generate cash flow report" }, { status: 500 });
  }
}
